...
const digimon = defineCollection({
  loader: glob({
    ...
    // Files are organised into rank subfolders (sss/, sssplus/) on disk,
    // but we want clean URLs like /digimon/<slug>/ — so derive the entry
    // id from the filename only, not the relative path.
